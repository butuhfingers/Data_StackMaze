...
public class MazeViewer extends Canvas {
    private Maze myMaze;
    ...
    public MazeViewer(Maze aMaze, int r, int c) {
        myMaze = aMaze;
        mazeRows = r;
        mazeCols = c;
    ...
    private void drawSquare(Node sq) {
        ...
        if (sq.getWall(Direction.NORTH).Exists()) {
        ...
        if (sq.getWall(Direction.SOUTH).Exists()) {
            g2d.draw(new Line2D.Float((tileSize * j),
        ...
        if (sq.getWall(Direction.EAST).Exists()) {
        ...
        if (sq.getWall(Direction.WEST).Exists()) {
    ...
    // is executed by the MazeViewer constructor when a new
    // MazeViewer is first instantiated.
    ...
    public void drawMaze() {
        ...
        for (int i = 0; i < mazeRows; i++) {
            for (int j = 0; j < mazeCols; j++) {
                Node sq = myMaze.squareAt(new Coordinate(i, j));
                colorSquare(g2d, sq);
    ...
    private void colorSquare(Graphics2D g2d, Node sq) {
        if (sq.isAbandoned()) {
